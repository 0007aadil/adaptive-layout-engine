...
const ORDER: ElementRole[] = ['branding', 'hero', 'primary', 'secondary', 'action']

/** Below this, a hero image is noise rather than art direction. */
const MIN_HERO_HEIGHT = 110

/**
 * Reads only the surface's geometry — never its id or name — so a profile the
 * resolver has never seen still gets a sensible archetype.
 */
export function selectTemplate(surface: SurfaceProfile): TemplateId {
...
/** Broadcast lower-thirds and other very wide frames: one row, brand left, action right. */
function placeStrip(elements: AdElement[], content: Rect, ctx: PlaceContext): StackResult {
  const hero = elements.find((el) => el.role === 'hero') as ImageElement | undefined
  const rest = elements.filter((el) => el !== hero)

  if (hero && content.height < MIN_HERO_HEIGHT) {
    return {
      ...EMPTY_STACK,
      overflow: [{ elementId: hero.id, reason: 'Frame is too short to carry a hero image' }],
    }
  }

  const branding = rest.filter((el) => el.role === 'branding')
  const action = rest.filter((el) => el.role === 'action')
  const copy = rest.filter((el) => el.role !== 'branding' && el.role !== 'action')
...
function track(ctx: PlaceContext, align: 'left' | 'center') {
  return {
    ...
    align,
    gap: ctx.gap,
    minTextSize: ctx.minTextSize,
    minTapTarget: ctx.minTapTarget,
  }
}
